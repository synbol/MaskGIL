...
import sys
sys.path.append("Network/tokenizer")
...
class MaskGIT(Trainer):
    ...
    def get_network(self, archi):
        ...
        if archi == "vit":
            ...
            """
                llamagen
            """
            model = GPT_models[self.args.gpt_model](
            ...
            checkpoint = torch.load('./vq_ds16_c2i.pt', map_location="cpu")
            # Load network
    ...
    def train_one_epoch(self, log_iter=500):
        ...
        for x, y in bar:
            ...
            if self.args.iter % log_iter ==0:
                with open("loss.txt", "a") as file:
                    file.write(str(self.args.iter) + ' ')
                    # file.write(" ")
                    file.write(' ' + str(np.array(window_loss).sum()))
                    file.write("\n")
            ...
            if self.args.iter % log_iter == 0 and self.args.is_master:
                ...
                save_image(gen_sample, "images/sample_{}.png".format(self.args.iter), nrow=4, normalize=True, value_range=(-1, 1))
    ...
    def sample(self, init_code=None, nb_sample=50, labels=None, sm_temp=1, w=3,
        ...
        with torch.no_grad():
            ...
            # print(_code.shape)
            qzshape = [_code.shape[0], 8, 16, 16]
            x = self.ae.decode_code(_code.reshape(_code.shape[0], -1), qzshape)
        self.vit.train()
